...
//         if (col_counts[rand_col] < MAX_ENEMY_IN_COL && row_counts[rand_row] < MAX_ENEMY_IN_ROW)
//         {
...
//     if (game_controller->alien_count >= 5) {
//         uart_puts("Error: Maximum number of aliens reached.\n");
//         return; // Early return if the array is full
//     }
...
//         if (col_counts[rand_col] < MAX_ENEMY_IN_COL && row_counts[rand_row] < MAX_ENEMY_IN_ROW) {
//             alien->position.x = pos_x[rand_col];
//             alien->position.y = 0;  // Start at the top of the screen
...
//         }
//     }
//     uart_puts("Target alien x: ");
//     // uart_puts(itoa(alien->target.x));
//     // uart_puts(", target alien y: ");
//     // uart_puts(itoa(alien->target.y));
//     // uart_puts("\n");
//     game_controller->aliens[game_controller->alien_count++] = *alien;
//     draw_alien(game_controller);
// }
...
//         // printf("Moving alien %d from y=%d to target y=%d\n", i, alien->position.y, alien->target.y); // Debug print
//         // uart_puts(i);
//         // uart_puts("\n");
//         // uart_puts("Moving alien from y=");
//         // uart_puts(itoa(alien->position.y));
//         // uart_puts("\n");
//         // uart_puts(" to target y=");
//         // uart_puts(itoa(alien->target.y));

//         if (!alien->reached_target) {
//             if (alien->position.y < alien->target.y) {
//                 alien->position.y += 2;  // Move down by 2 units per frame
//                 all_reached = 0;
//             } else {
...
//         // Handle other game activities
//         wait_msec(1);  // For approximately 60 FPS
//     }
// }
...
void move_aliens_to_positions(GameController *game_controller)
{
    ...
    for (int i = 0; i < game_controller->alien_count; i++)
    {
        ...
        if (!alien->reached_target)
        {
            clear_image(alien->position.x, alien->position.y, 130, 109, epd_bitmap_background_allArray[0]);
            if (alien->position.y < alien->target.y)
            {
...
void game_loop(GameController *game_controller)
{
    init_all_enemies(game_controller); // Make sure aliens are initialized before the loop starts
...
{
    for (int i = 0; i < 5; i++)
    { // Assuming 5 is the total number of aliens you can have
