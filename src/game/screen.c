      ...
      for (int i = 0; i < game_controller->bullet_on_screen_count; i++) {
        for (int j = 0; j < game_controller->spaceship.bullet_bonus + 1; j++) {
          if (game_controller->spaceship.bullets[i][j].name != NULL) {
            move_bullet(game_controller, i, 30);
          }
        }
      }
      fire_timer++;
  ...
  while (1) {
    ...
    case 's':
      ...
      manage_command(game_controller,
                     "Moved down to select another stage level", c, 1);
      break;
    case '\n':
...
void lose_screen(GameController *game_controller, int seconds) {
  ...
  while (1) {
    ...
    case 'y':
      manage_command(game_controller,
                     "Redirected to the game screen from lose result screen", c,
                     1);
      in_game_screen(game_controller);
      break;
    case 'n':
...
void win_final_screen(GameController *game_controller, int seconds) {
  ...
  while (1) {
    ...
    case 'n':
      manage_command(
          game_controller,
          "Redirected to the welcome screen from win final result screen", c,
          1);
      welcome_screen(game_controller);
      break;
    default:
...
void welcome_screen(GameController *game_controller) {
  ...
  while (1) {
    ...
    case '1':
      manage_command(game_controller,
                     "Redirected to the stage screen from welcome screen", c,
                     1);
    ...
    case '2':
      manage_command(
          game_controller,
          "Redirected to the spaceship selection screen from welcome screen", c,
          1);
...
void manage_command(GameController *game_controller, char *log, char *c,
                    int is_positive) {
